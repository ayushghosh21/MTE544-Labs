...
class Bagreader(Node):
    ...
    def laser_callback(self, msg):
        ...
        self.transform_laser(robot_pose)
    ...
    def map_to_cartesion(self, map_image):
        x = 1
    ...
    def lidar_to_cartesian(self, robot_pose, delta_base_lidar=[0, 0, 0]):
        ...
        ### TODO MAKE MORE INEFICIENT --------------
        #x_r = np.abs(x_r)
        #x = x.astype (int)
        #point_x[point_x > self.map_max_x] = -1
        #point_x[point_x < -self.map_max_x] = -1

        #y_r = np.abs(y_r)
        #y = y.astype (int)
        #point_y[point_y < -self.map_max_y] = -1
        #point_y[point_y > self.map_max_y] = -1
