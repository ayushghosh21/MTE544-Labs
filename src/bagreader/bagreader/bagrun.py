...
def particle_filter(posterior_samples:list, inputs: None):
    """
    ...
    for i in range(NUM_PARTICLES):
        ...
        w_k:list = predicted_samples[i][2] # probability distribution
        resampled_sample = np.random.choice(predicted_samples, w_k)
...
def likelihood_field(self,predicted_sample):
    ...
    lidar_pints: an occupancy grid in map dimensions
    Note: accesses zk_measurements and occupancy map
    """
